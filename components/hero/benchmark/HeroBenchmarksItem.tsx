    ...
    return (
      <div className="p-2 bg-black/40">
        ...
        <p className="desc capitalize">
          ...
          <span>{props.payload[0].value.toFixed(2)}</span>
        </p>
      </div>
    );
  }
...
const HeroBenchmarksItem = ({
  name,
  result,
}: {
  name: string;
  result: HeroBenchmarksItemProps[];
}) => {
  return (
    <div>
      <ResponsiveContainer width="100%" height={250}>
        <AreaChart width={400} height={250} data={result} id={name}>
          <defs>
            <linearGradient id={`${name}-UV`} x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="20%"
                stopColor={COLORS[name.toUpperCase()]}
                stopOpacity={0.8}
              />
              <stop
                offset="95%"
          ...
          <Area
            ...
            stroke={COLORS[name.toUpperCase()]}
            fillOpacity={1}
            fill={`url(#${name}-UV)`}
          />
        </AreaChart>
      </ResponsiveContainer>
      <div className="py-1 text-gray-400 text-right uppercase text-xls">
        {name.replaceAll("_", " ")}
      </div>
    </div>
  );
