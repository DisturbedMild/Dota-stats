...
const HeroMatchupsTab = ({ currentHero }: HeroDetailsTabList) => {
  ...
  const calculatedHeroMatchupsWinrateWithAdvantage = heroMatchups.map(
    (heroMatchup: Matchup) => {
      return {
        winrate: Number(
          ((heroMatchup.wins * 100) / heroMatchup.games_played).toFixed(2),
        ),
        advantage: calculateWilsonScore(
          heroMatchup.wins,
          heroMatchup.games_played,
        ),
        ...heroMatchup,
      };
    },
