...
const TalentCircle = ({ talentLevel }: TalentLevel) => {
  return (
    <div className="p-1 rounded-full basis-[30px] h-[30px] flex items-center justify-center bg-black/40 shadow-center shadow-amber-300 text-amber-300">
      {talentLevel}
    </div>
  );
};
...
const TalentsPopup = ({ talents }: TalentsPopupProps) => {
  const [updatedTalentsList, setUpdatedTalentsList] = useState<Talent[] | null>(
    null,
  );
  ...
  const addLevelsToArray = useCallback(() => {
    ...
    for (let i = 0; i < updatedTalentsArray.length; i++) {
      if (updatedTalentsArray[i].desc && updatedTalentsArray[i].level > level) {
        ...
        updatedTalentsArray.splice(i + 1, 0, {
          desc: "",
          level,
  ...
  return (
    <div className="absolute -bottom-20 right-14 flex items-center p-1 w-[440px] cursor-pointer z-10 border border-gray-900 bg-slate-800">
      <Image
      ...
      <div className="w-full z-2 flex flex-wrap justify-center gap-x-2 gap-y-4 items-center text-center text-white">
        {updatedTalentsList &&
          updatedTalentsList.map((talent: Talent) => {
            const id = uuidv4();
