...
const AbilityPopup = ({ ability }: AbilityPopupProps) => {
  ...
  return (
    <div className="absolute z-20 top-10 right-14 flex items-center p-1 w-[340px] cursor-pointer border border-gray-900 bg-slate-800 transition-all">
      <div className="p-2 w-full bg-slate-700">
        ...
        {(behavior || bkbpierce || dispellable || dmg_type) && (
          <div className="flex flex-col py-2 text-white text-xs border-b border-gray-500">
            {behavior && (
              <div>
                TARGET:{" "}
                {typeof behavior === "string" ? behavior : behavior.join(" / ")}
              </div>
            )}
        ...
        <p className="mt-2 text-xs text-white mb-2 pb-2 border-b border-gray-500">
          {desc}
        </p>
        {attrib.map((attr, i) => {
          return (
            <div
              key={`${dname}_skill_${attr.header}-${i}`}
        ...
        {lore && (
          <div className="mt-2 p-2 text-xs text-white bg-gray-500/20">
            {lore}
          </div>
        )}
