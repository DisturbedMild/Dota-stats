...
const HeroMatchesItem = ({ match }: HeroMatchesItemProps) => {
  ...
  const KDAPercentages = useMemo(
    () => calcKDALineWidth({ kills, assists, deaths }),
  ...
  return (
    <div className="flex items-center px-6 py-2 border-t border-b border-gray-200/10">
      ...
      <div className="flex flex-col w-4/12 text-xls">
        <span className={match.radiant_win ? "text-success" : "text-error"}>
          {match.radiant_win ? "Win Match" : "Lost Match"}
        </span>
      ...
      <div className="w-3/12 text-xls">
        ...
        <div className="w-full flex mt-2">
          <span
            className={`inline-block h-1 bg-success`}
            style={{ width: `${KDAPercentages.killsPercentage}%` }}
          ></span>
          <span
            className={`inline-block h-1 bg-error`}
            style={{ width: `${KDAPercentages.deathsPercentage}%` }}
          ></span>
          <span
            className={`inline-block h-1 bg-[#ffffff99]`}
            style={{ width: `${KDAPercentages.assistsPercentage}%` }}
          ></span>
        </div>
      </div>
    </div>
