...
const heroDetails = ({ currentHero }: HeroDetailsProps) => {
  let baseAttack;
  if (currentHero) {
    baseAttack = calcBaseDamage(
      currentHero.primary_attr,
      currentHero.base_str,
      currentHero.base_int,
      currentHero.base_agi,
    );
  }
        ...
        <>
            ...
            {baseAttack && (
              <>
                <div className="flex justify-between bg-secondary/30 px-4 py-3">
                  ...
                  <span>
                    {(baseAttack + currentHero.base_attack_min).toFixed(0)} -{" "}
                    {(baseAttack + currentHero.base_attack_max).toFixed(0)}
                  </span>
                </div>
          ...
          <div className="w-1/4">
            ...
            <div className="flex justify-between bg-secondary/70 px-4 py-3">
              ...
              <span>
                {(
                  currentHero.base_health_regen +
                  currentHero.base_str * 0.09
                ).toFixed(1)}
              </span>
            </div>
            <div className="flex justify-between bg-secondary/30 px-4 py-3">
            ...
            <div className="flex justify-between bg-secondary/70 px-4 py-3">
              ...
              <span>
                {(
                  currentHero.base_mana_regen +
                  currentHero.base_int * 0.05
                ).toFixed(1)}
              </span>
            </div>
          </div>
          <div className="w-1/4">
            <div className="flex justify-between bg-secondary/30 px-4 py-3">
              ...
              <span>
                {(
                  currentHero.base_armor +
                  currentHero.base_agi * 0.167
                ).toFixed(1)}
              </span>
            </div>
            <div className="flex justify-between bg-secondary/70 px-4 py-3">
