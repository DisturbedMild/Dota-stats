          ...
          return <div key={Math.random() * 1000}>
            {currentItemBuff ? (
              <div className="relative w-max">
                <Image src={currentItemBuff.img} loader={imageLoader} width={28} height={28} alt={currentItemBuff.dname} />
                <span className="absolute right-1 bottom-0">{stacks}</span>
              </div>
            ) : null}
            {currentAbilityBuff ? (
              <div className="relative w-max">
                <Image src={currentAbilityBuff.img} loader={imageLoader} width={28} height={28} alt={currentAbilityBuff.dname} />
                <span className="absolute right-1 bottom-0">{stacks}</span>
              </div>
            ) : null}
          </div>
