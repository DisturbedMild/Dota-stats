  ...
  if (currentActiveItem === null)
    return (
      <div className="flex flex-col justify-center p-2 w-3/12 bg-secondary/30 text-center transition-all">
        <h2 className="text-4xl text-white/20">
          You still didn&apos;t chose an item.
        </h2>
      </div>
    );
    ...
    <div className="w-3/12 h-max bg-secondary/30 transition-all">
      <div className="grid grid-cols-66-160 gap-x-4 px-1.5 py-3">
        ...
        <div className="flex flex-col w-48">
          <span className="text-xls">
            {currentActiveItem.dname.toUpperCase()}
          </span>
      ...
      <div
        className={`flex flex-col mb-3 px-1.5 pb-1 text-xls ${
          (currentActiveItem.behavior ||
            currentActiveItem.dmg_type ||
            currentActiveItem.bkbpierce) &&
      ...
      >
        {currentActiveItem.behavior && (
          <div>
            ...
            {typeof currentActiveItem.behavior === "string" && (
              <span>{currentActiveItem.behavior}</span>
            )}
            {Array.isArray(currentActiveItem.behavior) &&
              currentActiveItem.behavior.map((behavior) => (
        ...
        {currentActiveItem.bkbpierce &&
          currentActiveItem.bkbpierce === "Yes" && (
            <div>
        ...
        {currentActiveItem.dispellable &&
          currentActiveItem.dispellable !== "No" && (
            <div>
              <span>DISPELLABLE: </span>
              <span
                className={`${currentActiveItem.dispellable === "Yes" ? "text-green-600" : "text-pink-800"}`}
              >
                {currentActiveItem.dispellable}
              </span>
            </div>
      ...
      <div className=" px-1.5 py-2 bg-gray-900/60">
        ...
        {currentActiveItem?.abilities?.map((ability) => {
          if (ability.type === "active") {
            return (
              <div className="mt-2" key={ability.title}>
                ...
                <div className="px-2 py-1 bg-black/20 text-xs bg-gradient-to-r from-[#40b91a99] to-[#020024]">
                  {ability.description}
                </div>
              </div>
            );
          ...
          return (
            <div className="my-2" key={ability.title}>
              ...
              <div className="px-2 py-1 bg-gradient-to-r from-[#2b2d69] to-[#02002499] text-xs">
                {ability.description}
              </div>
            </div>
          );
        ...
        {currentActiveItem.hint?.length > 0 && (
          <div className="flex flex-col my-2 px-2 py-1 bg-black/30 text-white/40 text-xls">
            {currentActiveItem.hint.map((hint) => (
